...
namespace PictureConsoler
{
	class Program
	{
		public const string Ext = ".pcuf", OldClassicExt = ".pcff", OldXExt = ".pcxf", MclExt = ".pclm";
		...
		private static void PrintHeader()
		{
			...
			version = $"v[{version.Remove(version.LastIndexOf('.'))}]";
		...
		private static void ReadData(string[] args)
		{
			...
			else
			{
				...
				PCX.MassColorsDeterminor.Logger?.Stop();
				Console.WriteLine();
		...
		private static void ReadFilterSelection()
		{
			...
			switch (ReadKeyPress(new HashSet<ConsoleKey>(3) { ConsoleKey.F1, ConsoleKey.F3, ConsoleKey.F5 }))
			{
				...
				case ConsoleKey.F5:
					...
					ReadSobelCap();
					ReadOhlSobelCompressFactor();
					ReadOhlSobelDivisionFactor();
					ReadOhlReverseFlag();
					ReadFlagSavePreconsoledResult();
					break;
			}
		}
		private static void ReadOhlSobelCompressFactor()
		{
			ohlCompressFactor = ReadDoubleParameter("Sobel-compress factor", 0.3, 0.0, 1.0);
		}
		private static void ReadOhlSobelDivisionFactor()
		{
		...
		private static void ReadOhlReverseFlag()
		{
			...
			double lowerValue, double upperValue)
		{
			...
			while (true)
			{
				...
				if (!double.TryParse(input, System.Globalization.NumberStyles.Float, culture, out double result))
				{
					Console.Write("Parsing failed. Correct: ");
					continue;
				}
		...
		private static ConsoleKey ReadKeyPress(HashSet<ConsoleKey> available_keys)
		{
			...
			while (!available_keys.Contains(key));
		...
		private static string ReadBmpPath()
		{
			string path;
			while (true)
			{
		...
		private static ushort ReadDbInterval()
		{
			ushort result;
		...
		private static Func<Bitmap, Bitmap> GetFrameFilter()
		{
			...
			{
				case Filter.Sobel:
					filterF = (frameBmp) => SobelFilter.Filter(frameBmp);
					break;
		...
		private static string GenerateUniqueSavePath(string main, string ext)
		{
			...
			if (file.Exists)
			{
				...
				if (file.Exists)
					throw new IOException("Failed to generate unique save path!");
			}
