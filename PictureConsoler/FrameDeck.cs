...
namespace PictureConsoler
{
	abstract class FrameDeck
	{
		...
		public void SaveConsoled(string savePath)
		{
			if (Frames.Length > 1)
			{
				GifBuilder cGifBuilder = new GifBuilder();
				foreach (Frame cFrame in Frames)
					cGifBuilder.AddFrame(cFrame.ToBitmap());
				cGifBuilder.Save(savePath);
			}
			else Frames[0].ToBitmap().Save(savePath, ImageFormat.Png);
		}
		...
		protected Color GetAverageUpperSectorColor(int x, int y, Bitmap bitmap)
		{
			...
			if (OddSymbolH)
				AddRGBFromSectorLine(x, y + SymbolH - 1, bitmap, ref r, ref g, ref b);
			r /= UpperHalfs;
			g /= UpperHalfs;
			b /= UpperHalfs;
